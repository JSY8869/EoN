...
class Train_reservation:
    ...
    def T_list(self):
        try:
            self.Train_List = open("C:/Users/정세영/Desktop/E-on/.vscode/TrainList.txt", 'r')
    ...
    def calculator_train(self):
        ...
        while True:
            ...
            if time1:
                ...
                break  
        del self.time2[0], time3[0],self.start[0],self.end[0]
            ...
            for n in range(len(self.line)):
                if self.line[n] == self.near_time[answer]:
                    if self.seat_train[n] == 0:
                        ...
                        self.line[n] = self.seat_change[n+1] + str(self.seat_train[n])
    ...
    def cancel_train(self):
        ...
        if cancel_input == 1:
            ...
            if self.now:
                print ("왼쪽부터 0번 ~ ",len(self.now)-1,"번 입니다.")
                print (self.now)
                cancel_number = int(input("몇 번을 취소하시겠습니까 ? : "))
                for g in range(len(self.line)):
                    if self.now[cancel_number] == self.line[g]:
                        self.seat_train[g] = self.seat_train[g] + 1
                        self.line[g] = self.seat_change[g+1] + str(self.seat_train[g])  
                        print ("예약이 취소되었습니다.")
                        del self.now[cancel_number]
            elif not self.now:
                print ("예매 된 내역이 없습니다.")
                self.train_menu()
        elif cancel_input == 3:
